...
class InMemoryPreferencesRepository implements IPreferencesRepository {
  ...
  async get(userId: string): Promise<AccessibilityPreferences> {
    return this.store.get(userId) ?? createDefaultPreferences()
  }
...
class FailingPreferencesRepository implements IPreferencesRepository {
  async get(): Promise<AccessibilityPreferences> {
    throw new Error('API unavailable')
  }

  async update(): Promise<AccessibilityPreferences> {
    throw new Error('API unavailable')
  }
}
...
describe('FallbackPreferencesRepository', () => {
  ...
  it('usa fallback quando a API falha no get', async () => {
    ...
    const repo = new FallbackPreferencesRepository(
      new FailingPreferencesRepository(),
      fallback,
    )

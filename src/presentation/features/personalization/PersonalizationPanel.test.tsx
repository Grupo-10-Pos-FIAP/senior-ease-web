...
describe('PersonalizationPanel', () => {
  ...
  it('oferece rótulos descritivos nos controles de fonte', async () => {
    ...
    expect(within(fontSizeGroup).getByRole('radio', { name: /letra muito grande/i })).toBeInTheDocument()
    expect(within(fontSizeGroup).getByRole('radio', { name: /letra pequena/i })).toBeInTheDocument()
  })
  ...
  it('exibe mensagem de sucesso após salvar', async () => {
    ...
    expect(
      await within(panel).findByText(/preferências salvas com sucesso/i),
